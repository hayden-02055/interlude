...
- 여기서 꼭 잡을 것:
프로토콜 레벨과 인덱서/UI 정책 레벨의 책임 분리

## 관리 원칙

1. 리스크는 제거보다 완화가 기본이다.
2. 프로토콜 레벨과 인덱서/UI 레벨의 책임을 분리한다.
...
|----|--------|--------|---------------------|-----------|------|
| R-01 | 상품/상점 스팸 등록 폭증 | High | 신규 등록 급증, 신고율 급증 | staking/deposit, rate limit, 인덱서 필터 | Protocol + Indexer |
| R-02 | 시빌 공격으로 평판 왜곡 | High | 동일 패턴 계정군 상호 리뷰 | portable reputation + 신뢰 그래프 + 가중치 | Trust Layer |
| R-03 | 검색 인덱서 중앙화 | High | 특정 인덱서 의존도 과다 | 다중 인덱서, 클라이언트 선택 라우팅 | Discovery Infra |
| R-04 | 불법/사기 상품 노출 | High | 법적 요청/사용자 피해 신고 | 프로토콜 중립 + 인덱서 정책 분리 + 증거 보존 | Policy + Legal |
...
| R-13 | PII 인덱서 해킹으로 캐시된 PII 유출 | Medium | 인덱서 서버 침해 | 인덱서 캐시 암호화(at rest), 위임 키 범위 제한, 인덱서 선택/교체 가능 | Indexer Ops |
...
- 인덱서 기본 정책을 `신뢰 점수 하한` 모드로 전환
- 공격 패턴 주소군 증거 스냅샷 기록

### R-03 인덱서 중앙화
- MCP 기본 검색 전략을 `multi-index quorum`으로 전환
- 공식 인덱서 장애/편향 공지 템플릿 발행
- 대체 인덱서 목록 자동 갱신

### R-04 불법/사기 상품
- 프로토콜 데이터는 보존, 인덱서 표시 정책만 조정
