...
Solana 블록체인 위에 탈중앙화 커머스 에코시스템을 구축하는 이유와 핵심 결정사항을 정리한다.

---

## 핵심 아이디어

현재 UCP의 중앙화된 구조를 Solana 스마트 컨트랙트로 탈중앙화한다.

최우선 목표는 결제 수단 교체가 **아니라** 다음 3가지다:

- **Permissionless market** — 누구나 상점/상품을 등록할 수 있는 열린 레지스트리
- **Verifiable discovery** — UCP discovery를 통한 표준화된 상점/상품 탐색
- **On-chain trust** — 가격 이력, 상태 전이, 주문 이벤트가 검증 가능한 신뢰 계층

**현재 UCP:**

```mermaid
flowchart LR
  P1["Platform (AI)"] <--> B1["Business<br/>(중앙화된 상점)"]
  B1 <--> PSP["PSP<br/>(Stripe 등)"]
...
```mermaid
flowchart LR
  P2["Platform (AI)"] <--> S["Solana<br/>Smart Contract"]
  S <--> M["탈중앙화된<br/>상품/발견/거래"]
  S --> C2["블록체인이 신뢰 계층<br/>상품 on-chain<br/>discovery verifiable<br/>주문 불변"]
```
...
| 7 | Buyer PII는 주문별 ephemeral key로 암호화 후 on-chain 임시 저장, Merchant 수신 확인 후 PDA closure | [pocket/](pocket/) |
| 8 | **Curator Badge**를 MerchantProfile에 임베딩 — 웹 인증서(CA) 모델 | [curator/](curator/) |
| 9 | 에코시스템 명칭은 **Interlude** — Pocket, Market, Curator, Badge | — |
...
```mermaid
flowchart LR
  P1["Phase 1<br/><b>Permissionless Registry</b><br/>Merchant·Product on-chain<br/>온체인 기반 discovery<br/>다중 Curator"]
  P2["Phase 2<br/><b>Verifiable Discovery + Reputation</b><br/>스팸·시빌 방어<br/>portable score<br/>정책 계층 분리"]
  P3["Phase 3<br/><b>Settlement Modules</b><br/>sol.usdc / sol.native<br/>Escrow·분쟁·환불 점진 도입"]
  P1 --> P2 --> P3
```

| Phase | 핵심 산출물 | 기간 (예상) |
|-------|------------|------------|
| 1 | Solana Program (registry), Market MCP Server, Curator Badge | 2-6주 |
| 2 | 스팸/시빌 방어, reputation scoring, 정책 계층 | 4-10주 |
| 3 | sol.usdc/sol.native 결제, Escrow, 분쟁/환불 | 6-14주 |

> 전체 로드맵과 마일스톤은 [operations/roadmap.md](operations/roadmap.md) 참조.
...
블록체인 자체가 "누구도 변조 못함"을 보장한다. 암호학적 서명은 지갑 서명으로 대체되며, Curator Badge가 MerchantProfile에 임베딩되어 AI Agent가 즉시 판매자 신뢰를 검증한다.
...
AI에게 위임된 Solana 지갑 (Squads 멀티시그). 한도 내에서 자동 결제하며, 블록체인이 한도를 강제한다.
...
상품 가격 변경이 모두 on-chain. AI가 "이 상품 어제보다 10% 올랐어요"를 자동 감지할 수 있다.
